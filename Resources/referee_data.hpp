...
struct UiInitData{
    ...
    uint16_t auto_shoot_target_Rect_size[2];//自瞄目标半长宽

    uint16_t robot_bound_line_left[2][2];  //车道线左
    uint16_t robot_bound_line_right[2][2];  //车道线右
    ...
    uint16_t attack_arch[2]; //受击弧线
    uint16_t attack_arch_radius[2];//受击弧线半径
    ...
    uint16_t balance_num_string_pos[2];     //平衡标号字符位置 x,y

    uint16_t balance_num_pos[2];     //平衡号码字符位置 x,y
    uint16_t minipc_states_pos[2];   //自瞄状态字符位置

    uint8_t refresh_times;//刷新计数，确保为static，初始化置0
    uint8_t hurt_refresh_interval;//刷新间隔
};

extern const UiInitData default_ui_init_data;
/* Exported variables --------------------------------------------------------*/
...
class Referee
{
    ...
    private:
        ...
        void uiAddComponents11(referee::InterGraphicStringPackage *pag);
        void uiAddComponents12(referee::InterGraphicStringPackage *pag);
