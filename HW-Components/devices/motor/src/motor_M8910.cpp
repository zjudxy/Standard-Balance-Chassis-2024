 ...
 * @note        None
 */
M8910::M8910(uint8_t id, const OptionalParams& optinal_params) : Motor()
{
  /* 变量检查 */
#pragma region
 ...
 */
State M8910::encode(uint8_t tx_data[8], uint32_t tx_msg_std_id)
{
  /* 变量检查 */
#pragma region
 ...
 */
State M8910::decode(const uint8_t rx_data[8], uint32_t rx_msg_std_id)
{
  /* 变量检查 */
#pragma region
...
#pragma endregion
  ...
  if (rx_msg_std_id != motor_info_.rx_id) {
    ...
    torq_ = raw2torq(raw);
    curr_ = raw2curr(raw);
    power_ = (static_cast<int16_t>((rx_data[6] << 8) | rx_data[7])) *
             1000.0f / 0x8000;
