 ...
 * @note        None
 */
GM6020::GM6020(uint8_t id, const OptionalParams& optinal_params) : Motor()
{
  /* 变量检查 */
#pragma region
...
/**
 * @brief       设定发给电调的期望报文原始值
 * @param        input: 发给电调的期望报文原始值
 * @retval       运行状态
 * @note        设定的期望值会自动被限制到允许的范围内，当前实际的设定值可以通
 *              过 getInput 方法查看
 */
State GM6020::setInput(float input)
{
...
State GM6020::set_input_type(InputType input_type)
{
  if (input_type == kInputTypeRaw) {
 ...
 */
State GM6020::encode(uint8_t tx_data[8], uint32_t tx_msg_std_id)
{
  /* 变量检查 */
#pragma region
 ...
 */
State GM6020::decode(const uint8_t rx_data[8], uint32_t rx_msg_std_id)
{
  /* 变量检查 */
#pragma region
...
#pragma endregion
  ...
  if (rx_msg_std_id != motor_info_.rx_id) {
    ...
    float raw = static_cast<float>(motor_info_.dir) *
                static_cast<int16_t>((rx_data[4] << 8) | rx_data[5]);
